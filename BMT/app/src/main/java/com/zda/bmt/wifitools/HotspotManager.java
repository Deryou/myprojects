...
public class HotspotManager {
    ...
    public boolean connectWifi(String ssid, String pwd, List<WifiItem> scanResults) {
        ...
        if (config == null) {
            ...
            final boolean isCurrentNetwork_ConfigurationStatus = config.status ==
            ...
            if (!isCurrentNetwork_ConfigurationStatus && !isCurrentNetwork_WifiInfo) {
                //连接已保存的WiFi
                String scanResultSecurity = Wifi.ConfigSec.getScanResultSecurity(result);
                final WifiConfiguration wcg = Wifi.getWifiConfiguration(mWifiManager, result,
                        scanResultSecurity);
                boolean connResult = false;
                if (wcg != null) {
                    connResult = Wifi.connectToConfiguredNetwork(mContext, mWifiManager, wcg,
                            false);
                }
                return connResult;
            } else {
     ...
     * @return
     */
    public boolean isConnectedStatue(ScanResult result) {
        if (result == null) {
            return false;
        }
        if (isAdHoc(result)) {
        ...
        WifiConfiguration config = Wifi.getWifiConfiguration(mWifiManager, result, security);
        if (config != null) {
            boolean isCurrentNetwork_ConfigurationStatus = config.status == WifiConfiguration.Status
                    .CURRENT;
            WifiInfo info = mWifiManager.getConnectionInfo();
    ...
    public ScanResult getResult(String ssid, List<WifiItem> scanResults) {
        if (scanResults == null || scanResults.size() == 0) {
            return null;
        }
        //匹配SSID相同的Wifi
     ...
     * @return
     */
    public boolean connectToNet(String pwd, ScanResult result) {
        //连接新wifi
        boolean connResult;
        int numOpenNetworksKept = Settings.Secure.getInt(mContext.getContentResolver(),
                Settings.Secure.WIFI_NUM_OPEN_NETWORKS_KEPT, 10);
     ...
     * @return
     */
    public boolean connectToConfiguredNet(ScanResult result) {
        if (isConnectedStatue(result)) {
            //连接已保存的WiFi
            String scanResultSecurity = Wifi.ConfigSec.getScanResultSecurity(result);
            final WifiConfiguration wcg = Wifi.getWifiConfiguration(mWifiManager, result,
                    scanResultSecurity);
