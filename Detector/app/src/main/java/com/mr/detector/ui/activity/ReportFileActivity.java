...
import com.mr.detector.adapter.ReportListAdapter;
 ...
 * 开启热点
 */
public class ReportFileActivity extends BaseActivity implements ReportListAdapter.OnItemClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        adapter = new ReportListAdapter(this, mFilelist);
...
//        File file = this.getExternalFilesDir(Environment.DIRECTORY_DOCUMENTS);
        mFilelist.clear();
        File file = new File(Util.getSDPaht(this)+ AppConstants.REPORT_FOLDER);
    ...
    public void click(View view) {
        switch (view.getId()) {
            ...
            case R.id.file_checked:
                for (int i = 0; i < mFilelist.size(); i++) {
                    if (adapter.getSelected(i) == false) {
                        adapter.setSelected(i);
                    }
                }
                break;
